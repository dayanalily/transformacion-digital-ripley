...
export class AuthService{
    ...
    async Indentify(email: string, password: string): Promise<Users | false>{
        let user = await this.usersRepository.findOne({ where: { email, active: true}})
        if(user){
    ...
    async IndentifyTwo(password: string): Promise<string>{
       //  let user = await this.usersRepository.findOne({ where: { email, active: true}})
        // if(user){
            let cryptPass = new EncryptDecrypt(KeysService.LOGIN).Ecrypt(password)
            // if(user.password == cryptPass){
